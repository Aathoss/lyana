...
func AddPlayer(ctx framework.Context) {
	...
	if ctx.Staff >= 1 {
		if len(ctx.MessageSplit) >= 2 {
			tagdiscord := ctx.MessageSplit[1]
			playermc := ctx.MessageSplit[2]
			...
			if err != nil {
				...
				modules.LogDiscord("[:x:] Une erreur c'est produits sur `" + ctx.MessageSplit[0] + " " + tagdiscord + " " + playermc + "`\n" + err.Error() + "`\n" + "uuid discord : " + user.User.ID)
				return
			}
