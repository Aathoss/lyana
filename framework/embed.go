...
func (e *Embed) TicketUserField(content [][]string) *Embed {
	...
	if len(content) != 0 {
		for i := 0; i <= len(content)-1; i++ {
			json.Unmarshal([]byte(content[i][3]), &retuInfo)
			if content[i][4] == "false" {
				e.AddField("N°"+content[i][0]+" :small_blue_diamond: Ticket : "+content[i][1]+" :small_blue_diamond: Résolu en : "+content[i][2], string(retuInfo["message"].(string)), false)
			}
			if content[i][4] == "true" {
				e.AddField("N°"+content[i][0]+" :small_blue_diamond: Ticket : "+content[i][1]+" :small_blue_diamond: Ouvert", string(retuInfo["message"].(string)), false)
			}

		}
		e.SetFooter("Vous pouvez voir en détail le ticket avec !ticket voir <id>")
	}
	if len(content) == 0 {
		e.SetDescription("Après de multiples recherches, nous n'avons trouvé aucun ticket.")
	}
